...
head(f_df)
f_df$department[f_df$department == "finishing " | f_df$department == "finishing"] <- 1
...
#PCR:
library(pls) 
fit_pcr <- pcr(actual_productivity ~ .,ncomp = 10,data=train_xy)
summary(fit_pcr)
...
cv_ridge <- cv.glmnet(actual_productivity ~ ., data = train_xy, alpha = 0)  # alpha = 0 is the ridge penalty
...
result <- data.frame (Algorithms  = c('Regression','Principal Component Regression',
                                      'Ridge Regression','Regression Decision Trees','Random Forest Regression'),
                      MAE = c(lr_mae,pcr_mae,ridge_mae,tree_mae,rf_mae),
                      MSE = c(lr_mse,pcr_mse,ridge_mse,tree_mse,rf_mse),
                      RMSE= c(lr_rmse,pcr_rmse,ridge_rmse,tree_rmse,rf_rmse)
)
result
print('The optimal one is Random Forest Regressor')
library(DT)

datatable(result, rownames = FALSE) %>%
  formatStyle(columns = "Algorithms", 
              backgroundColor = "lightblue") %>%
  formatStyle(columns = "MAE", 
